...
export const UpdateFacilities = ({
  ...
  const [newFacility, setNewFacility] = useState({
    ...
    facility_description: facility ? facility.facility_description : "",
    facility_image: null,
  });
  const [errMsg, setErrMsg] = useState("");
  const [image, newImage] = useState(null);
  useEffect(() => {
    if (facility && facility.facility_image) {
      setNewFacility((prev) => ({
        ...prev,
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://bjerg.pythonanywhere.com/api/admin/facilities/${facility.id}`,
        formData,
        {
          headers: {
      ...
      <div className="flex flex-col gap-[10px]">
        ...
        <div className="w-full flex justify-between ">
          <FieldInput
            ...
            type={"text"}
            value={newFacility.facility_description || ""}
            handleChange={(e) => {
              setNewFacility((prev) => ({
                ...prev,
        ...
        {newFacility.facility_image && (
          <div className="mt-2">
            <img
              src={`https://bjerg.pythonanywhere.com/media/${newFacility.facility_image}`}
              alt="Facility Image"
